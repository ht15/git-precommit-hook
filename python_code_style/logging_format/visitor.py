...
class LoggingVisitor(NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        if node.args and isinstance(node.args[0], ast.Str):
            left_count = node.args[0].s.count('%')
            right_count = len(node.args) - 1
            # ignore *args
            if left_count != right_count and node.starargs is None:
                self.violations.append((node, ARGS_NOT_MATCH_VIOLATION))
